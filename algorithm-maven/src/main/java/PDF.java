        ...
        try {
            ...
            for (int n = 0; n < this.similarStructure.size(); n++) {
                if (fileIndex == this.similarStructure.get(n).file1) {
                    Text similarS = new Text(String.format("%s. Line %d to line %d in file %s has similar structure where located line %d to line %d in file %s", Integer.toString(++count), this.similarStructure.get(n).lines1[0], this.similarStructure.get(n).lines1[1], this.fileInfos.get(fileIndex).getFileName(), this.similarStructure.get(n).lines2[0], similarStructure.get(n).lines2[1], this.fileInfos.get(this.similarStructure.get(n).file2).getFileName()));
